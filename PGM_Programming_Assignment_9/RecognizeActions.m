...
% YOUR CODE HERE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[P1 logli1 ClassProb1 PairProb1] = EM_HMM(datasetTrain(1).actionData, datasetTrain(1).poseData, G, ...
                                          datasetTrain(1).InitialClassProb, datasetTrain(1).InitialPairProb, maxIter);


[P2 logli2 ClassProb2 PairProb2] = EM_HMM(datasetTrain(2).actionData, datasetTrain(2).poseData, G, ...
                                          datasetTrain(2).InitialClassProb, datasetTrain(2).InitialPairProb, maxIter);


[P3 logli3 ClassProb3 PairProb3] = EM_HMM(datasetTrain(3).actionData, datasetTrain(3).poseData, G, ...
                                          datasetTrain(3).InitialClassProb, datasetTrain(3).InitialPairProb, maxIter);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for iclass = 1:3
		  ...
		  if G(j,1) == 1
			  ...
			  for k = 1:K
				  ...
				  logEmissionProb(i,k) = logEmissionProb(i,k) + ...
                                         lognormpdf(data(j,1),mu_y,P(iclass).clg(j).sigma_y(k)) + ...
                                         lognormpdf(data(j,2),mu_x,P(iclass).clg(j).sigma_x(k)) + ...
                                         lognormpdf(data(j,3),mu_a,P(iclass).clg(j).sigma_angle(k));
			  end
		  else
			  for k = 1:K
				  logEmissionProb(i,k) = logEmissionProb(i,k) + ...
                                         lognormpdf(data(j,1),P(iclass).clg(j).mu_y(k),P(iclass).clg(j).sigma_y(k)) + ...
                                         lognormpdf(data(j,2),P(iclass).clg(j).mu_x(k),P(iclass).clg(j).sigma_x(k)) + ...
                                         lognormpdf(data(j,3),P(iclass).clg(j).mu_angle(k),P(iclass).clg(j).sigma_angle(k));
			  end
		  end
	  end
  end
  ...
  for i = 1:I
	  ...
	  F = repmat(struct('var',[],'card',[],'val',[]),1,2*m); % 1 is P(S1), 2 to m is P(Si/Si-1) and m+1 to 2m P(S/O)
	  ...
	  loglikelihood(i,iclass) = loglikelihood(i,iclass) + logsumexp(PCalibrated.cliqueList(1).val);
  end
end
